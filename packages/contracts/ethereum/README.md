...
- `solidity/src/GoBridge.sol` — L1 bridge contract. `depositToAztecPublic` accepts FJ + emits the L1→L2 message picked up by the Aztec rollup. Used by `@aztec-kit/bridge` and by the deploy framework’s fee bridging (`@aztec-kit/common/deploy`).
