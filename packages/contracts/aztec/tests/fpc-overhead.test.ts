...
describe("FPC gas overhead", () => {
  ...
  it("FPC calls emit exactly the expected side effects", () => {
    ...
    // hashes + 2 message logs) and the SlotNote pop nullifier. The v5
    // tx-request protocol nullifier is emitted by every private tx, so it
    // is already present in the steady-state baseline and cancels out here.
