...
import { type DeployState, loadState, saveState } from "./state.ts";
import type {
  ActionStep,
  ContractStep,
  Ctx,
...
class DeploymentRun<C extends Steps> {
  ...
  private static validateSpec<C extends Steps>(spec: DeploymentSpec<C>): void {
    ...
    for (const [alias, step] of steps) {
      ...
      if (step.initializerArgs && step.deferredInitializerArgs) {
        throw new Error(
          `Contract "${alias}" declares both initializerArgs and deferredInitializerArgs — pick one.`,
        );
      }
      if (!isDeferred(step)) {
        continue;
  ...
  public async reportPlan(): Promise<DeployPlan> {
    ...
    for (const alias of this.execAliases) {
      ...
      const address =
        step.kind === "contract" ? step.deployer(this.resolver) : step.from(this.resolver);
      accountUsedBy.add(address.toString());
    }
    const stepStatus = (
      alias: string,
      step: StepSpec<C>,
    ): DeployPlan["steps"][number]["status"] => {
      if (step.kind === "action") {
  ...
  public reportSummary(): void {
    const summary: DeploySummary = {
      ...
      contracts: [...this.contractAddresses].map(([alias, address]) => ({
        ...
        address,
        status:
          getOrThrow(this.contractSteps, alias, "contract").mode === "register"
            ? "registered"
            : "published",
      })),
  ...
  private async computeClassIds(): Promise<void> {
    ...
    await Promise.all(
      [...this.contractSteps].map(async ([alias, step]) => {
        ...
        if (!id) {
          id = getContractClassFromArtifact(step.contract.artifact).then(
            (contractClass) => contractClass.id,
          );
  ...
  private isPublished(alias: string): Promise<boolean> {
    ...
    if (!cached) {
      cached = this.wallet
        ...
        .then((metadata) => metadata.isContractPublished);
    ...
    if (!cached) {
      cached = (async () => {
        try {
          ...
          return (
            (await publicFeeJuiceBalance(this.wallet, recipient, step.from(this.resolver))) >=
            step.threshold
          );
        } catch {
  ...
  private buildLayers(): string[][] {
    ...
    return scheduleLayers(
      this.execAliases,
      execDeps,
      (id) => getOrThrow(this.steps, id, "step").kind === "action",
    );
  }
