...
const deployStateSchema = z.object({
  ...
  pendingClaims: z
    .record(
      z.string(),
      z.object({ claimAmount: z.string(), claimSecret: z.string(), messageLeafIndex: z.string() }),
    )
    .default({}),
});
...
export interface DeployState {
  ...
  pendingClaims: Record<
    string,
    { claimAmount: string; claimSecret: string; messageLeafIndex: string }
  >;
}
...
export function loadState(dir: string): DeployState {
  ...
  try {
    ...
    // VENDORED delta: no `{ cause }` (kit TS lib target).
    throw new Error(`Failed to read deploy state at ${statePath(dir)}: ${String(error)}`);
  }
  try {
    return deployStateSchema.parse(JSON.parse(raw));
  } catch (error) {
    throw new Error(
      `Corrupt deploy state at ${statePath(dir)} (${String(error)}). Fix or remove it manually — removing ` +
        `abandons any pending bridge claims recorded in it (their funds become unclaimable).`,
    );
  }
}
