...
import { bridgeFeeJuice, waitForL1ToL2Message } from "./bridging.ts";
import type { AccountFunding, DeployReporter } from "./reporter.ts";
...
export async function publicFeeJuiceBalance(
  ...
  const { result } = await FeeJuiceContract.at(wallet)
    .methods.balance_of_public(address)
    .simulate({ from });
...
function resolveL1Config(
  ...
  if (l1RpcUrl === undefined || l1ChainId === undefined) {
    throw new Error(
      "Bridging on a non-local network requires `l1RpcUrl` and `l1ChainId` (no hardcoded defaults).",
    );
  }
  return {
    l1RpcUrl,
...
export async function prepareFeeSession(opts: PrepareFeeSessionOpts): Promise<FeeSession> {
  ...
  // VENDORED delta: derivation inlined (upstream reuses its package's local-network helper).
  ...
  const sessions = new Map<
    string,
    { sponsored: boolean; claim?: FeeJuicePaymentMethodWithClaim }
  >();
