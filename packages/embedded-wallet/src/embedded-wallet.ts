...
export class EmbeddedWallet extends EmbeddedWalletBase {
  ...
  async warmTagCache(opts?: { windowSize?: number }): Promise<void> {
    ...
    if (this.#warmInFlight) {
      // Coalesce — another warm is already running. Remember the
      // trigger so we re-fire once the current cycle finishes.
      this.#warmRequestedAgain = true;
      return;
    }
    if (this.#accountKeys.size === 0) return;
    ...
    try {
      const tips = (await (
        proxy as unknown as { getL2Tips: () => Promise<unknown> }
      ).getL2Tips()) as { proposed?: { hash: { toString(): string } } } | undefined;
      if (!tips?.proposed?.hash) return;
      anchorHash = tips.proposed.hash;
    } catch {
      return;
    }
    ...
    // Fire one more cycle to pick up whatever changed.
   ...
   * a secretKey) into `#accountKeys`. Derives the complete address +
   * ivsk locally — no PXE round-trip needed. The base wallet has
   * already called `pxe.registerAccount` for us before this fires.
   */
